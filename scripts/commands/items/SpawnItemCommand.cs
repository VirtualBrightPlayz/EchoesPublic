...
public class SpawnItemCommand : AdminConsoleCommandButton
{

    public override string Command { get; } = "SpawnItem";
    
    public override string[] Alias { get; } = [];
    
    public override string CommandDescription { get; } = "Spawns an Item";
    
    public override string ButtonLabel => "Spawn Item";
    ...
    public override bool Execute(PlayerConsole console, string[] args, out string response)
    {
        ...
        foreach (NetworkPlayer player in players)
        {
            ...
            itemObj.ModelEnabled = true;
            itemObj.SV_Teleport(player.PlayerPosition + Vector3.Up, player.PlayerRotation);
        }
        
        response = $"Item {itemPreset?.ResourceName} spawned close to {string.Join(", ", players.Select(p => p.username))}";
        return true;
    }
    ...
    public override void OnPressed(AdminHUD admin)
    {
        ...
        {
            admin.RunWithSelectedPlayers($"spawnitem {{0}}");
        }
        
        admin.RunWithSelectedPlayers($"spawnitem {{0}} {selectedItem[0]}");
    }
}