...
namespace FragLabs.Audio.Codecs
{
    ...
    public class OpusDecoder : IDisposable
    {
        ...
        public unsafe float[] DecodeFloat(byte[] inputOpusData, int dataLength, out int decodedLength)
        {
            ...
            IntPtr decodedPtr;
            float[] decoded = new float[MaxDataBytes /* / sizeof(float)*/];
            int frameCount = FrameCount(MaxDataBytes /* / sizeof(float)*/);
            int length = 0;
            fixed (float* bdec = decoded)
            {
                ...
                if (inputOpusData != null)
                    length = API.opus_decode_float(_decoder, inputOpusData, dataLength, decodedPtr, decoded.Length / OutputChannels, 0);
                else
        ...
        public int FrameCount(int bufferSize)
        {
            ...
            return (int)((float)bufferSize / OutputSamplingRate /* / OutputChannels*/ * 1000f);
            // return OutputSamplingRate - bufferSize;
            int bitrate = 16;
